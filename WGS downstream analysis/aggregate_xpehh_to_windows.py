...
def main():
    parser = argparse.ArgumentParser(description="Aggregate XP-EHH SNP scores to windows.")
    parser.add_argument("-i", "--input", required=True, help="normalized XP-EHH file")
    ...
    score_col = find_column(df, ["norm_xpehh", "xpehh"])
    crit_col  = find_column(df, ["crit"])

    if chrom_col is None or pos_col is None or score_col is None:
        raise ValueError(
            f"Unexpected XP-EHH file format. Need columns like chr, pos, norm_xpehh/xpehh.\n"
    ...
    for chrom in df["CHROM"].dropna().unique():
        ...
        for s in starts:
            ...
            row = {
                ...
                "start": s,
                "end": e,
                "XPEHH_mean": win["SCORE"].mean(),
                "XPEHH_mean_abs": win["ABS_SCORE"].mean(),
                "XPEHH_max": win["SCORE"].max(),
                "XPEHH_min": win["SCORE"].min(),
                "XPEHH_max_abs": win["ABS_SCORE"].max(),
                "n_snp": win.shape[0]
            }
